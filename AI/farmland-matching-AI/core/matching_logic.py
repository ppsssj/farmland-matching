...
# --- 핵심 매칭 로직 ---
def calculate_distance_score(farmland: Farmland, buyer: Buyer, max_score: int = 100) -> float:
    """거리 점수를 계산합니다. 5km 이내 만점, 13.15km 초과 0점."""
    distance = haversine_distance(farmland.landLat, farmland.landLng, buyer.buyerLat, buyer.buyerLng)
    
    max_dist = 13.15  # 서비스 한계 반경(km)
    min_dist = 5      # 만점 반경(km)
    ...
    if distance <= min_dist:
        ...
        # min_dist에서 max_dist 사이를 선형적으로 감소
        return max_score * (1 - (distance - min_dist) / (max_dist - min_dist))
...
def calculate_crop_score(farmland: Farmland, buyer: Buyer, max_score: int = 100) -> float:
...
def calculate_facility_score(farmland: Farmland, max_score: int = 100) -> float:
    """시설 점수를 계산합니다."""
    score = 0
    if farmland.landWater: score += 10
    if farmland.landElec: score += 10
    if farmland.landStorage: score += 10
    if farmland.landHouse: score += 10
    if farmland.landRoad: score += 10
    if farmland.landWellRoad: score += 10
    if farmland.landBus: score += 10
    if farmland.landCar: score += 10
    if farmland.landMachine: score += 10
    if farmland.landFence: score += 10
    return min(score, max_score)

def calculate_area_score(farmland: Farmland, buyer: Buyer, max_score: int = 100) -> float:
