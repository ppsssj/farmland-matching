...
def match_by_hybrid_model(buyer: Buyer):
    ...
    # 1) K-Means 클러스터 예측 (작물 + 좌표 기반)
    trust_profile = buyer.trustProfile
    
    # 작물 벡터 생성
    crop_vector = preprocessors['crop_mlb'].transform([trust_profile.interestCrop])[0]
    
    # 좌표 벡터 생성 (학습때 사용한 Scaler로 동일하게 정규화)
    coord_scaler = preprocessors['coord_scaler']
    coord_vector = coord_scaler.transform([[buyer.buyerLat, buyer.buyerLng]])[0]

    # 두 벡터를 결합하여 최종 입력 벡터 생성
    applicant_vector = np.hstack([crop_vector, coord_vector])
